...
if ($res == 0) 
     ...
     {
            echo $lastCustIdAdded = mysqli_insert_id($dbcnx); 
            $todaysDate =  date("Y-m-d");
            $totalPrice = 0;

$dbcnx1 = mysqli_connect("localhost", "root", "", "TyreStore");     

       
// Check connection

if (mysqli_connect_errno($dbcnx1 )){
echo "Failed to connect to MySQL: " .mysqli_connect_error();
exit();}
...
if ($_POST['submitdetails'] == "NEXT") {
        ...
        //Inserting data into database
$sql1 = "INSERT INTO Sales(CustId,TotalPrice,SaleDate) VALUES($lastCustIdAdded,$totalPrice, '$todaysDate')";

$res = mysqli_query($dbcnx1, $sql1);
$lastSaleIdAdded = mysqli_insert_id($dbcnx1);
echo   $lastSaleIdAdded;
if ($res == 0) 
      {
        echo("<p>Error registering: " . mysqli_error(). "</p>");
      }
     ...
     {
            header("location:salesItems.php?lastsaleid=$lastSaleIdAdded & lastCustIdAdded=$lastCustIdAdded");
